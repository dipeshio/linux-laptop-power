...
if [ -f "$VIVALDI_DESKTOP" ]; then
    ...
    sed -i 's|Exec=/usr/bin/vivaldi-stable|Exec=/usr/bin/vivaldi-stable --enable-features=TurnOffStreamingMediaCachingOnBattery,UseOzonePlatform --disable-backgrounding-occluded-windows|' "$LOCAL_VIVALDI"
